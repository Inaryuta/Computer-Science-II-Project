...
# ---------- Ventana principal de Función Plegamiento ----------
class FuncionPlegamientoWindow(QMainWindow):
    ...
    def initUI(self):
        self.setWindowTitle("Función Hash - Plegamiento")
        ...
        titulo = QLabel("FUNCIÓN HASH - PLEGAMIENTO")
        ...
        self.digitos_spin.setRange(2, 10)  # Mínimo 2 para poder plegar
    ...
    # ---------- Función hash de plegamiento ----------
    def hash_plegamiento(self, clave):
        """
        Calcula la posición usando el método de plegamiento.
        Divide la clave en grupos de 2 dígitos y los suma (ignorando acarreos).
        Luego toma módulo con la capacidad.
        """
        # Convertir clave a string de dígitos (ya viene como string)
        # Asegurar que tenga al menos 2 dígitos
        if len(clave) < 2:
            return int(clave) % self.capacidad  # fallback
        ...
        suma = sum(grupos)
        # Opcional: ignorar acarreo (tomar solo los últimos dígitos de la suma)
        # Para simplificar, tomamos suma % (10**len(str(self.capacidad)))? No, mejor módulo con capacidad.
        return suma % self.capacidad

    # ---------- Métodos de probing para estrategias abiertas ----------
    def probar_lineal(self, clave, intento):
        h = self.hash_plegamiento(clave)
    ...
    def probar_doble_hash(self, clave, intento):
        h1 = self.hash_plegamiento(clave)
    ...
    def insertar_clave(self):
        ...
        pos = self.hash_plegamiento(clave)
        ...
        if self.estrategia_actual in ("Lineal", "Cuadrática", "Doble Hash"):
            ...
            while intento <= max_intentos:
                ...
                if self.estructura[pos] == clave_int:
                    ...
                    QMessageBox.information(self, "Resultado", f"✅ Clave {clave} encontrada en posición {pos+1} (probing).")
                    return
                if self.estructura[pos] is None:
                    break
                intento += 1
            ...
            for i, lista in enumerate(self.estructura_anidada):
                if clave_int in lista:
                    ...
                    self._resaltar_posicion(i)
                    QMessageBox.information(self, "Resultado", f"✅ Clave {clave} encontrada en posición {i+1}, {tipo} #{idx_col}.")
                    return
            QMessageBox.information(self, "Resultado", f"❌ Clave {clave} no encontrada.")
        else:
            for i, val in enumerate(self.estructura):
                if val == clave_int:
                    self._resaltar_posicion(i)
                    QMessageBox.information(self, "Resultado", f"✅ Clave {clave} encontrada en posición {i+1}.")
                    return
            QMessageBox.information(self, "Resultado", f"❌ Clave {clave} no encontrada.")
    ...
    def guardar_estructura(self):
        ...
        nombre_defecto = f"hash_plegamiento_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
