...
class DialogoColisiones(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Colisión detectada")
        self.setModal(True)
        self.setStyleSheet("""
            QDialog {
                background-color: #f0f8ff;
...
# ---------- Ventana principal de Función Módulo ----------
class FuncionModWindow(QMainWindow):
    def __init__(self, volver_a_busquedas, volver_a_principal):
        ...
        self.capacidad = 0
        self.digitos = 4
        self.estructura = []          # Lista de tamaño capacidad, con valores o None
        self.estructura_anidada = []   # Lista de listas para colisiones (por posición)
        self.historial = []            # Lista de acciones para deshacer
        self.ultima_estrategia = None  # "Arreglo anidado" o "Lista encadenada"
    ...
    def initUI(self):
        self.setWindowTitle("Función Hash - Módulo")
        ...
        titulo = QLabel("FUNCIÓN HASH - MÓDULO")
    ...
    def hash_mod(self, clave):
        """Calcula la posición según módulo."""
        return int(clave) % self.capacidad
    ...
    def insertar_clave(self):
        ...
        # Verificar si hay espacio total (principal + anidada)
        total_ocupadas = sum(1 for x in self.estructura if x is not None) + sum(len(lst) for lst in self.estructura_anidada)
        if total_ocupadas >= self.capacidad:
        ...
        # Verificar duplicado (en principal y anidada)
        ...
        pos = self.hash_mod(clave)

        if self.estructura[pos] is None:
            # Sin colisión
            self.estructura[pos] = clave_int
            self.historial.append(("insertar", pos, clave_int, None))  # None indica sin colisión
            if self.ultima_estrategia:
                # Si hay estrategia previa, mantener la vista especial
                if self.ultima_estrategia == "Arreglo anidado":
                    self.actualizar_vista_anidada()
                elif self.ultima_estrategia == "Lista encadenada":
            ...
            if estrategia == "Arreglo anidado":
                ...
                self.historial.append(("insertar", pos, clave_int, "Arreglo anidado"))
                self.actualizar_vista_anidada()
            else:  # Lista encadenada
                # En lista encadenada, la estructura principal ya tiene un valor, y los siguientes se guardan en la lista anidada
                self.estructura_anidada[pos].append(clave_int)
                self.historial.append(("insertar", pos, clave_int, "Lista encadenada"))
                self.actualizar_vista_encadenada()

            QMessageBox.information(self, "Éxito", f"Clave {clave} insertada con estrategia {estrategia}.")

    def actualizar_vista_anidada(self):
        """Vista especial para arreglo anidado (celdas adicionales pegadas)."""
        self.limpiar_vista()

        # Calcular el máximo de colisiones por posición
        max_colisiones = max((len(lst) for lst in self.estructura_anidada), default=0)

        # Crear un layout vertical que contendrá filas
        ...
        for i in range(self.capacidad):
            ...
            lbl_principal = QLabel(str(val) if val is not None else "")
            ...
            # Rellenar con celdas vacías hasta el máximo para mantener alineación
    ...
    def actualizar_vista_encadenada(self):
        """Vista especial para lista encadenada (con flechas)."""
        self.limpiar_vista()
    ...
    def buscar_clave(self):
        ...
        for i, val in enumerate(self.estructura):
            if val == clave_int:
                ...
                QMessageBox.information(self, "Resultado", f"Clave {clave} encontrada en posición {i+1} (principal).")
                return

        # Buscar en anidada
        for i, lista in enumerate(self.estructura_anidada):
            if clave_int in lista:
                # Podríamos resaltar la posición principal y mostrar mensaje
                QMessageBox.information(self, "Resultado", f"Clave {clave} encontrada en colisión de la posición {i+1}.")
                return

        QMessageBox.information(self, "Resultado", f"Clave {clave} no encontrada.")

    def _resaltar_posicion(self, idx):
        """Resalta una posición específica en la vista actual."""
        # Este método depende del tipo de vista; simplificado: solo mensaje
        pass
    ...
    def eliminar_clave(self):
        ...
        for i, lista in enumerate(self.estructura_anidada):
            if clave_int in lista:
                ...
                self.historial.append(("eliminar", i, clave_int, self.ultima_estrategia))
                self._actualizar_segun_estrategia()
    ...
    def _actualizar_segun_estrategia(self):
        """Actualiza la vista según la última estrategia usada."""
        if self.ultima_estrategia == "Arreglo anidado":
            self.actualizar_vista_anidada()
        elif self.ultima_estrategia == "Lista encadenada":
            self.actualizar_vista_encadenada()
    ...
    def guardar_estructura(self):
        ...
        nombre_defecto = f"hash_mod_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def cargar_estructura(self):
        ...
        self.ultima_estrategia = datos.get("ultima_estrategia")
        self.historial = []
    ...
    def deshacer(self):
        ...
        if tipo == "insertar":
            ...
            if estrategia is None:
                ...
                self.estructura_anidada[pos].sort()  # Opcional, para mantener orden
