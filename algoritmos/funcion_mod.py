...
# ---------- Diálogo para elegir estrategia de colisión (con nuevas opciones) ----------
class DialogoEstrategia(QDialog):
    def __init__(self, parent=None):
        ...
        # Estrategias cerradas (con listas)
...
class FuncionModWindow(QMainWindow):
    ...
    def actualizar_vista_normal(self):
        ...
        for i, idx in enumerate(self.indices_reales):
            ...
            # No mostrar los marcadores DELETED como valores
    ...
    def probar_doble_hash(self, clave, intento):
        ...
        h1 = módulo, h2 = 1 + (clave % (capacidad-2)) para asegurar que sea coprimo con capacidad."""
        h1 = self.hash_mod(clave)
        # Segunda función hash: un número primo menor que capacidad, por ejemplo capacidad-1, pero aseguramos que no sea 0
    ...
    def insertar_clave(self):
        ...
        # Contamos los no None y no DELETED
        ...
        if self.estrategia_actual in ("Arreglo anidado", "Lista encadenada"):
            ...
            # En abierto, buscar en toda la tabla (incluyendo posiciones de probing)
        ...
        if self.estrategia_actual in ("Arreglo anidado", "Lista encadenada"):
            ...
            if self.estrategia_actual == "Arreglo anidado":
                ...
                self.estructura_anidada[pos].append(clave_int)  # (se puede ordenar si se desea)
            self.historial.append(("insertar", pos, clave_int, self.estrategia_actual))
            self._actualizar_segun_estrategia()
    ...
    def buscar_posicion_por_clave(self, clave_int):
        ...
        # Primero, si es estrategia abierta, hay que buscar con el mismo probing
        if self.estrategia_actual in ("Lineal", "Cuadrática", "Doble Hash"):
            h = self.hash_mod(str(clave_int))
            intento = 0
            max_intentos = self.capacidad
            while intento <= max_intentos:
                if self.estrategia_actual == "Lineal":
                ...
                if self.estructura[pos] is None:
                    # Encontramos un hueco vacío, la clave no está
                    return None
                intento += 1
            return None
        else:
            # Para estrategias cerradas, buscar en principal
            ...
            for i, lista in enumerate(self.estructura_anidada):
                if clave_int in lista:
                    return i  # Devolvemos la posición principal (aunque esté en anidada)
            return None
    ...
    def buscar_clave(self):
        ...
        if self.estrategia_actual in ("Lineal", "Cuadrática", "Doble Hash"):
            ...
            # Búsqueda en principal y anidada
            for i, val in enumerate(self.estructura):
                if val == clave_int:
            ...
            for i, lista in enumerate(self.estructura_anidada):
                if clave_int in lista:
                    ...
                    self._resaltar_posicion(i)  # resaltar la principal
                    QMessageBox.information(
                        self,
                        "Resultado",
    ...
    def _resaltar_posicion(self, idx):
        ...
        try:
            ...
            # Si no está visible, no se puede resaltar
            pass

    def _reset_label_styles(self):
        """Restaura los estilos normales de todas las celdas según su estado."""
        for i, idx in enumerate(self.indices_reales):
            lbl = self.labels[i]
            valor = self.estructura[idx] if idx < len(self.estructura) else None
            if valor is not None and valor is not self.DELETED:
    ...
    def eliminar_clave(self):
        ...
        if self.estrategia_actual in ("Lineal", "Cuadrática", "Doble Hash"):
            # Búsqueda con probing para eliminar
            h = self.hash_mod(clave)
            intento = 0
            max_intentos = self.capacidad
            while intento <= max_intentos:
    ...
    def _actualizar_segun_estrategia(self):
        """Actualiza la vista según la estrategia actual."""
        if self.estrategia_actual == "Arreglo anidado":
    ...
    def actualizar_vista_anidada(self):
        """Vista especial para arreglo anidado."""
        self.limpiar_vista()
        ...
        for i in range(self.capacidad):
            ...
            # Rellenar con celdas vacías hasta el máximo
    ...
    def guardar_estructura(self):
        ...
        # Convertir marcadores DELETED a None para JSON
    ...
    def cargar_estructura(self):
        ...
        # Restaurar estructura convirtiendo "__DELETED__" a DELETED
    ...
    def deshacer(self):
        ...
        if tipo == "insertar":
            if estrategia in ("Arreglo anidado", "Lista encadenada"):
                # Eliminar de la lista anidada (si está en ella) o de principal
                ...
                # Abierto: marcar como borrado
                self.estructura[pos] = self.DELETED
        elif tipo == "eliminar":
            if estrategia in ("Arreglo anidado", "Lista encadenada"):
                # Restaurar en principal o anidada según corresponda
                # Aquí necesitaríamos saber si estaba en principal o anidada. Simplificamos: suponemos que si la posición está en principal, se restaura allí; si no, en anidada.
                if self.estructura[pos] is None:
                    self.estructura[pos] = valor
                else:
                    self.estructura_anidada[pos].append(valor)
            else:
                # Abierto: restaurar el valor
                self.estructura[pos] = valor
