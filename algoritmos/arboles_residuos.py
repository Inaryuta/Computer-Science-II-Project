...
from controladores.tree_residuos_controller import ArbolResiduosController


class ArbolesResiduosWindow(QMainWindow):
    def __init__(self, volver_a_busquedas, volver_a_principal):
        ...
        self.controller = ArbolResiduosController()

        self.setWindowTitle("Árboles de Residuos")
        ...
        titulo = QLabel("ÁRBOLES DE RESIDUOS")
    ...
    def dibujar_arbol(self):
        ...
        brush_link = QBrush(QColor("#b0b0b0"))      # gris para nodos de enlace
        brush_hoja = QBrush(QColor("#2ecc71"))      # verde para nodos hoja
        ...
        def draw(node, x, y, offset, depth):
            ...
            # Texto del nodo (si es hoja, la letra; si es enlace, '*')
            if node.is_link:
                txt = "*"
            else:
