...
# ---------- Diálogo para elegir posiciones de truncamiento ----------
class DialogoPosiciones(QDialog):
    def __init__(self, longitud_clave, num_posiciones, parent=None):
        """
        longitud_clave: número de dígitos de la clave (ej. 4)
        num_posiciones: número de dígitos que se deben seleccionar (generalmente igual a longitud_clave o el necesario para el hash)
        """
        super().__init__(parent)
        self.longitud_clave = longitud_clave
        self.num_posiciones = num_posiciones
        self.posiciones = []
        self.setWindowTitle("Seleccionar posiciones para truncamiento")
        self.setModal(True)
        self.setStyleSheet("""
            QDialog {
                background-color: #f0f8ff;
        ...
        lbl_info = QLabel(f"Seleccione {num_posiciones} posiciones (de 1 a {longitud_clave}) para formar el hash:")
        layout.addWidget(lbl_info)

        # Crear checkboxes para cada posición
        self.checkboxes = []
        grid = QGridLayout()
        for i in range(1, longitud_clave + 1):
            cb = QCheckBox(f"Posición {i}")
            cb.setChecked(False)
            self.checkboxes.append(cb)
            grid.addWidget(cb, (i-1)//5, (i-1)%5)  # 5 columnas
    ...
    def validar_y_aceptar(self):
        ...
        for i, cb in enumerate(self.checkboxes):
            if cb.isChecked():
                seleccionadas.append(i + 1)  # posiciones 1-based
        if len(seleccionadas) != self.num_posiciones:
            QMessageBox.warning(self, "Error", f"Debe seleccionar exactamente {self.num_posiciones} posiciones.")
            return
        self.posiciones = seleccionadas
        self.accept()
...
# ---------- Ventana principal de Función Truncamiento ----------
class FuncionTruncamientoWindow(QMainWindow):
    ...
    def __init__(self, volver_a_busquedas, volver_a_principal):
        ...
        self.capacidad = 0
        self.digitos = 4
        self.posiciones = []           # Lista de posiciones seleccionadas para truncamiento (ej. [1,3,5])
    ...
    def initUI(self):
        self.setWindowTitle("Función Hash - Truncamiento")
        ...
        titulo = QLabel("FUNCIÓN HASH - TRUNCAMIENTO")
    ...
    # ---------- Función hash de truncamiento ----------
    def hash_truncamiento(self, clave):
        """
        Calcula la posición usando las posiciones previamente seleccionadas.
        clave: string de dígitos.
        Retorna: índice entre 0 y capacidad-1.
        """
        if not self.posiciones:
            # Si no hay posiciones, se toma como 0 (esto no debería ocurrir)
            return 0
        # Extraer los dígitos en las posiciones indicadas (1-based)
        digitos = ''.join([clave[p-1] for p in self.posiciones])  # p-1 porque clave es índice 0
        hash_int = int(digitos)
    ...
    def probar_lineal(self, clave, intento):
        ...
        h = self.hash_truncamiento(clave)
    ...
    def probar_doble_hash(self, clave, intento):
        ...
        h1 = hash_truncamiento, h2 = 1 + (clave % (capacidad-2)) para asegurar que sea coprimo con capacidad."""
        h1 = self.hash_truncamiento(clave)
    ...
    def crear_estructura(self):
        ...
        # Primero preguntar posiciones de truncamiento
        dlg_pos = DialogoPosiciones(digitos, digitos, parent=self)  # Suponemos que se seleccionan tantas posiciones como dígitos tiene la clave
        if dlg_pos.exec() != QDialog.Accepted:
            return
        self.posiciones = dlg_pos.get_posiciones()
    ...
    def insertar_clave(self):
        ...
        pos = self.hash_truncamiento(clave)
    ...
    def buscar_posicion_por_clave(self, clave_int):
        ...
        if self.estrategia_actual in ("Lineal", "Cuadrática", "Doble Hash"):
            h = self.hash_truncamiento(str(clave_int))
            intento = 0
            max_intentos = self.capacidad
            while intento <= max_intentos:
                if self.estrategia_actual == "Lineal":
    ...
    def guardar_estructura(self):
        ...
        nombre_defecto = f"hash_truncamiento_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def cargar_estructura(self):
        ...
        if not all(k in datos for k in ("rango", "digitos", "capacidad", "estructura", "estructura_anidada", "posiciones")):
