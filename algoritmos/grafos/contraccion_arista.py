# algoritmos/grafos/contraccion_arista.py
import json
...
class ContraccionAristaWindow(QMainWindow):
    def __init__(self, volver_a_grafos, volver_a_principal):
        ...
        self.setWindowTitle("Contracción de Arista")
        ...
        titulo = QLabel("CONTRACCIÓN DE ARISTA")
        ...
        # Columna derecha: contracción
        right_widget = QWidget()
        right_layout = QVBoxLayout(right_widget)
        right_layout.setSpacing(8)

        lbl_contraccion = QLabel("CONTRACCIÓN")
        lbl_contraccion.setStyleSheet("font-weight: bold; color: #003366;")
        right_layout.addWidget(lbl_contraccion, alignment=Qt.AlignCenter)

        # Selección de arista
        sel_layout = QHBoxLayout()
        sel_layout.addWidget(QLabel("Arista a contraer:"))
        self.combo_arista = QComboBox()
        self.combo_arista.setFixedWidth(180)
        ...
        self.visual_contraccion = VisualizadorGrafo("Grafo Contraído", es_editable=False)

        for vis in (self.visual_original, self.visual_contraccion):
        ...
        btn_guardar_cont = QPushButton("Guardar Contracción")
        btn_guardar_cont.setStyleSheet(self._button_style("#3498db", "white"))
        btn_guardar_cont.clicked.connect(self.guardar_contraccion)
        btn_limpiar_cont = QPushButton("Limpiar Contracción")
        btn_limpiar_cont.setStyleSheet(self._button_style("#95a5a6", "white"))
        btn_limpiar_cont.clicked.connect(self.limpiar_contraccion)
        ...
        extra_layout.addWidget(btn_guardar_cont)
        extra_layout.addWidget(btn_limpiar_cont)
    ...
    def eliminar_arista(self):
        ...
        if not self.grafo._aristas:
            ...
            return
        etiquetas = self.grafo._etiquetas
        # Para eliminar, usamos el diálogo pero luego eliminamos por primera coincidencia (o podríamos elegir índice)
        dlg = DialogoArista(n, self, etiquetas)
        if dlg.exec():
    ...
    def actualizar_combo(self):
        ...
        for idx, (u, v, p) in enumerate(self.grafo._aristas):
            etiq_u = self.grafo._etiquetas.get(u, str(u+1))
            etiq_v = self.grafo._etiquetas.get(v, str(v+1))
            texto = f"{etiq_u} - {etiq_v}"
            if p != 1:
                texto += f" [peso {p}]"
            self.combo_arista.addItem(texto, idx)  # guardamos el índice de la arista
    ...
    def contraer_arista(self):
        ...
        if n < 2:
            ...
            return
        idx_arista = self.combo_arista.currentData()
        if idx_arista is None:
            DialogoClave(0, "Error", "mensaje", self, "Selecciona una arista.").exec()
            return
        u, v, peso_contraido = self.grafo._aristas[idx_arista]
        if u == v:
            DialogoClave(0, "Error", "mensaje", self, "No se puede contraer un bucle.").exec()
            return
        # Ordenar para que u < v
        if u > v:
            u, v = v, u
        etiq_u = self.grafo._etiquetas.get(u, str(u+1))
        etiq_v = self.grafo._etiquetas.get(v, str(v+1))
        nueva_etiq = f"{etiq_u},{etiq_v}"
        ...
        for i in range(n):
            if i == u:
                ...
                mapeo[i] = nuevo_idx
                nuevo_idx += 1
        vertices_contraccion = n - 1

        # Etiquetas
        etiquetas_contraccion = {}
        for old, new in mapeo.items():
            if old != v:  # solo una vez por vértice original (v se mapea al mismo que u)
                if old == u:
                    etiquetas_contraccion[new] = nueva_etiq
                else:
                    etiquetas_contraccion[new] = self.grafo._etiquetas.get(old, str(old+1))

        # Recolectar aristas, excluyendo la contraída
        aristas_contraccion = []
        pesos_contraccion = []
        for idx, (a, b, p) in enumerate(self.grafo._aristas):
            if idx == idx_arista:
                continue  # omitir la arista contraída
            na = mapeo[a]
            nb = mapeo[b]
            # Si la arista original conectaba u y v (distinta a la contraída) -> se convierte en bucle
            if (a == u and b == v) or (a == v and b == u):
                # Crear bucle en el nuevo vértice
                aristas_contraccion.append((na, na))  # bucle
                pesos_contraccion.append(p)
            else:
                aristas_contraccion.append((na, nb))
                pesos_contraccion.append(p)

        self.visual_contraccion.set_grafo(vertices_contraccion, aristas_contraccion, etiquetas_contraccion, pesos_contraccion)
        DialogoClave(0, "Contracción completada", "mensaje", self,
                     f"Arista ({etiq_u} - {etiq_v}) contraída.\n"
                     f"Vértices resultantes: {vertices_contraccion}\nAristas: {len(aristas_contraccion)}").exec()

    def guardar_contraccion(self):
        if self.visual_contraccion.num_vertices == 0:
            DialogoClave(0, "Error", "mensaje", self, "No hay contracción para guardar.").exec()
            return
        datos = {
            'vertices': self.visual_contraccion.num_vertices,
            'aristas': self.visual_contraccion.aristas,
            'etiquetas': self.visual_contraccion.etiquetas,
            'pesos': self.visual_contraccion.pesos
        }
        ruta, _ = QFileDialog.getSaveFileName(self, "Guardar Contracción", "", "JSON (*.json)")
        if ruta:
            with open(ruta, 'w', encoding='utf-8') as f:
                json.dump(datos, f, indent=4)
            DialogoClave(0, "Éxito", "mensaje", self, "Contracción guardada.").exec()
