...
class UnionGrafosWindow(QMainWindow):
    def __init__(self, volver_a_grafos, volver_a_principal):
        ...
        self.setWindowTitle("Unión de Grafos")
        ...
        # Botón calcular
        self.btn_calcular = QPushButton("∪  CALCULAR UNIÓN")
        ...
        self.visual_union = VisualizadorGrafo("Unión (G1 ∪ G2)", es_editable=False)

        for vis in (self.visual1, self.visual2, self.visual_union):
        ...
        btn_guardar_union = QPushButton("Guardar Unión")
        btn_guardar_union.setStyleSheet(self._button_style("#3498db", "white"))
        btn_guardar_union.clicked.connect(self.guardar_union)
        btn_limpiar_union = QPushButton("Limpiar Unión")
        btn_limpiar_union.setStyleSheet(self._button_style("#95a5a6", "white"))
        btn_limpiar_union.clicked.connect(self.limpiar_union)
        ...
        extra_layout.addWidget(btn_guardar_union)
        extra_layout.addWidget(btn_limpiar_union)
    ...
    def calcular_union(self):
        ...
        DialogoClave(0, "Unión calculada", "mensaje", self,
                     f"Unión calculada:\n\n"
                     f"Vértices totales: {len(todas)}\n"
    ...
    def guardar_union(self):
        if self.visual_union.num_vertices == 0:
            ...
            return
        datos = {
            'vertices': self.visual_union.num_vertices,
            'aristas': self.visual_union.aristas,
            'etiquetas': self.visual_union.etiquetas
        }
        ruta, _ = QFileDialog.getSaveFileName(self, "Guardar Unión", "", "JSON (*.json)")
        if ruta:
            with open(ruta, 'w', encoding='utf-8') as f:
                json.dump(datos, f, indent=4)
            DialogoClave(0, "Éxito", "mensaje", self, "Unión guardada.").exec()

    def limpiar_union(self):
        self.visual_union.set_grafo(0, [], {})
