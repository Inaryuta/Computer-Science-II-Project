...
class SumaAnilloGrafosWindow(QMainWindow):
    def __init__(self, volver_a_grafos, volver_a_principal):
        ...
        self.setWindowTitle("Suma de Anillo de Grafos")
        ...
        self.visual_suma = VisualizadorGrafo("Suma Anillo (G1 ⊕ G2)", es_editable=False)
        ...
        # No crear grafos automáticamente
    ...
    def calcular_suma_anillo(self):
        ...
        def aristas_con_etiquetas(grafo):
            ...
            for (u, v, peso) in grafo._aristas:
                ...
                if etiq_u and etiq_v:
                    if etiq_u > etiq_v:
                        etiq_u, etiq_v = etiq_v, etiq_u
                    resultado.append((etiq_u, etiq_v, peso))
            return resultado
        ...
        for a in aristas_g1:
            ...
            pesos_por_arista[key] = a[2]
        for a in aristas_g2:
            key = (a[0], a[1])
            conteo[key] = conteo.get(key, 0) + 1
            if key not in pesos_por_arista:
                pesos_por_arista[key] = a[2]
        ...
        for key, cnt in conteo.items():
            if cnt == 1:
                ...
                if nu > nv:
                    nu, nv = nv, nu
                aristas_finales.append((nu, nv))
                pesos_dict[(nu, nv)] = pesos_por_arista.get(key, 1)

        self.visual_suma.set_grafo(len(todas), aristas_finales, nuevas_etiquetas, pesos_dict)

        total_g1 = len(aristas_g1)
        total_g2 = len(aristas_g2)
        comunes = len([k for k, cnt in conteo.items() if cnt == 2])
        DialogoClave(0, "Suma Anillo calculada", "mensaje", self,
                    f"Suma Anillo calculada:\n\n"
                    f"Vértices totales: {len(todas)}\n"
                    f"  • De G1: {len(etiq1)}\n"
                    f"  • De G2: {len(etiq2)}\n\n"
                    f"Aristas en G1: {total_g1}\n"
                    f"Aristas en G2: {total_g2}\n"
                    f"Aristas comunes: {comunes}\n"
                    f"Aristas en suma anillo: {len(aristas_finales)}").exec()

    def guardar_suma_anillo(self):
        if self.visual_suma.num_vertices == 0:
            DialogoClave(0, "Error", "mensaje", self, "No hay suma anillo para guardar.").exec()
            return
        datos = {
        ...
        ruta, _ = QFileDialog.getSaveFileName(self, "Guardar Suma Anillo", "", "JSON (*.json)")
        if ruta:
            with open(ruta, 'w', encoding='utf-8') as f:
                json.dump(datos, f, indent=4)
            DialogoClave(0, "Éxito", "mensaje", self, "Suma anillo guardada.").exec()
