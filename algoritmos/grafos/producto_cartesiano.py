...
class ProductoCartesianoWindow(QMainWindow):
    def __init__(self, volver_a_grafos, volver_a_principal):
        ...
        self.setWindowTitle("Producto Cartesiano de Grafos")
        ...
        titulo = QLabel("PRODUCTO CARTESIANO")
        ...
        self.btn_calcular = QPushButton("▶️  CALCULAR\nG₁ □ G₂")
        ...
        self.visual_producto = VisualizadorGrafo("Producto Cartesiano G₁ □ G₂", es_editable=False)
    ...
    def crear_grafo(self, num):
        if num == 1:
            ...
            # Etiquetas con letras mayúsculas A, B, C...
            for i in range(n):
                self.grafo1._etiquetas[i] = chr(65 + i)
            self.actualizar_visual(1)
    ...
    def calcular_producto(self):
        ...
        # Mapeo (i1, i2) -> índice en el producto
        mapping = {}
        idx = 0
        etiquetas_producto = {}
        for i in range(n1):
        ...
        # Caso 1: misma i, arista en G₂
        for i in range(n1):
            for (j1, j2, _) in self.grafo2._aristas:
                u = mapping[(i, j1)]
                v = mapping[(i, j2)]
                aristas_prod.add(tuple(sorted((u, v))))

        # Caso 2: misma j, arista en G₁
        for j in range(n2):
            for (i1, i2, _) in self.grafo1._aristas:
                u = mapping[(i1, j)]
                v = mapping[(i2, j)]
                aristas_prod.add(tuple(sorted((u, v))))
        ...
        msg = (f"G₁: {n1} vértices, {len(self.grafo1._aristas)} aristas\n"
               ...
               f"Producto: {vertices_prod} vértices, {len(aristas_prod)} aristas")
        DialogoClave(0, "Producto Cartesiano", "mensaje", self, msg).exec()
    ...
    def guardar_producto(self):
        ...
        ruta, _ = QFileDialog.getSaveFileName(self, "Guardar Producto", "", "JSON (*.json)")
        if ruta:
