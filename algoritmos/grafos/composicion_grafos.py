...
class ComposicionGrafosWindow(QMainWindow):
    def __init__(self, volver_a_grafos, volver_a_principal):
        ...
        self.setWindowTitle("Composición de Grafos")
        ...
        titulo = QLabel("COMPOSICIÓN DE GRAFOS")
        ...
        """)
        self.btn_calcular.clicked.connect(self.calcular_composicion)
        ...
        self.visual_composicion = VisualizadorGrafo("Composición G₁ ∘ G₂", es_editable=False)

        for vis in (self.visual_g1, self.visual_g2, self.visual_composicion):
        ...
        btn_guardar_comp = QPushButton("Guardar Composición")
        btn_guardar_comp.setStyleSheet(self._button_style("#3498db", "white"))
        btn_guardar_comp.clicked.connect(self.guardar_composicion)
    ...
    # ========== Composición de grafos (producto lexicográfico) ==========
    def calcular_composicion(self):
        ...
        for i in range(n1):
            ...
            for j in range(n2):
                ...
                mapping[(i, j)] = idx
                etiquetas_comp[idx] = f"({etiq1},{etiq2})"
                idx += 1

        vertices_comp = n1 * n2

        # Conjuntos de aristas existentes (normalizadas)
        aristas_g1 = set()
        ...
        for (i1, j1), idx1 in mapping.items():
            for (i2, j2), idx2 in mapping.items():
                if idx1 >= idx2:
                    continue
                # Condición 1: (i1,i2) es arista en G₁
                arista_g1 = tuple(sorted((i1, i2)))
                cond1 = arista_g1 in aristas_g1
                # Condición 2: i1 == i2 y (j1,j2) es arista en G₂
                cond2 = (i1 == i2) and (tuple(sorted((j1, j2))) in aristas_g2)
                if cond1 or cond2:
                    aristas_comp.add(tuple(sorted((idx1, idx2))))

        # Visualizar
        self.visual_composicion.set_grafo(vertices_comp, list(aristas_comp), etiquetas_comp, {})
        msg = (f"G₁: {n1} vértices, {len(self.grafo1._aristas)} aristas\n"
               f"G₂: {n2} vértices, {len(self.grafo2._aristas)} aristas\n\n"
               f"Composición: {vertices_comp} vértices, {len(aristas_comp)} aristas")
        DialogoClave(0, "Composición calculada", "mensaje", self, msg).exec()

    def guardar_composicion(self):
        if self.visual_composicion.num_vertices == 0:
            DialogoClave(0, "Error", "mensaje", self, "No hay composición para guardar. Calcule primero.").exec()
            return
        datos = {
            'vertices': self.visual_composicion.num_vertices,
            'aristas': self.visual_composicion.aristas,
            'etiquetas': self.visual_composicion.etiquetas,
            'pesos': self.visual_composicion.pesos
        }
        ruta, _ = QFileDialog.getSaveFileName(self, "Guardar Composición", "", "JSON (*.json)")
        if ruta:
            with open(ruta, 'w', encoding='utf-8') as f:
                json.dump(datos, f, indent=4)
            DialogoClave(0, "Éxito", "mensaje", self, "Composición guardada.").exec()
