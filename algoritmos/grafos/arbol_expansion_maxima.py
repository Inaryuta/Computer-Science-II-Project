# algoritmos/grafos/arbol_expansion_maxima.py
import json
...
from controladores.visualizador_grafo import VisualizadorGrafo, VisualizadorGrafoColoreable
...
# ══════════════════════════════════════════════════════════════════════
def kruskal_max(n: int, aristas: list[tuple]) -> tuple[list[int], list[int]]:
    """
    Algoritmo de Kruskal para árbol de expansión MÁXIMA.
    Devuelve (ramas, cuerdas) donde:
        ramas  = lista de índices de arista que pertenecen al árbol máximo
        cuerdas = lista de índices de arista que NO pertenecen al árbol máximo
    """
    if n == 0:
        return [], []
    ...
    aristas_con_idx.sort(key=lambda x: x[0], reverse=True)  # ← clave: reverse
    ...
    for peso, u, v, idx in aristas_con_idx:
        if u == v:
            cuerdas.append(idx)  # bucles nunca están en el árbol
        elif union(u, v):
            ramas.append(idx)
        else:
            cuerdas.append(idx)
...
#  Ventana principal
# ══════════════════════════════════════════════════════════════════════
class ArbolExpansionMaximaWindow(QMainWindow):
    def __init__(self, volver_a_grafos, volver_a_principal):
        ...
        self.setWindowTitle("Árbol de Expansión Máxima")
    ...
    # ──────────────────────────────────────────────────────────────────
    #  Construcción UI (idéntica a mínima)
    # ──────────────────────────────────────────────────────────────────
    def _build_ui(self):
        central = QWidget()
        ...
        self.visual_ramas = VisualizadorGrafoColoreable("Árbol de Expansión Máxima (Ramas)", es_editable=False)
        ...
        controles_layout.addWidget(self._sep())
        br = QPushButton("▶ Calcular árbol de expansión máxima")
        ...
        lbl_titulo = QLabel("Información del árbol máximo")
    ...
    def _make_header(self) -> QFrame:
        ...
        titulo = QLabel("ÁRBOL DE EXPANSIÓN MÁXIMA (KRUSKAL)")
    ...
    # ──────────────────────────────────────────────────────────────────
    #  Acciones (igual que mínima, pero con el algoritmo max)
    # ──────────────────────────────────────────────────────────────────
    ...
    def _calcular(self):
        ...
        ramas, cuerdas = kruskal_max(n, aristas)
    ...
    def _mostrar_ramas(self):
        ...
        colores_aristas = {i: "#3498db" for i in range(len(aristas_ramas))}  # azul para máximo
    ...
    def _mostrar_cuerdas(self):
        ...
        colores_aristas = {i: "#95a5a6" for i in range(len(aristas_cuerdas))}  # gris
